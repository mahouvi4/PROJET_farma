...
<div class="card">
    <div class="card-title"><marquee>xxxx</marquee></div> 
  <div class="card-header">ADMIN PHARMACY LOGIN</div>
  <div class="card-body">
    <div class="info2"></div>
    <br><br>
    <form method="POST" id="login_pharmo11">
        <div class="row">
            <div class="col-6">
               <div class="row">
           ...
           <div class="group-control">
            <button type="button" class="btn btn-primary mt-2" style="width:70px;margin-left:-1px" onclick="login1_control2_phar({{$log2->id}})">Login</button> <br><br>
...
@section('scripts')
<script>
                function login1_control2_phar(id){
                    ...
                    $.ajax({
                   type:"post",
                   url:"{{url('admin/formulaire1_login2_ph')}}/"+id,
                   data:formdata,
                   processData:false,
                   contentType:false,
                   success:function(response){
                       if(response.status==200){
                        window.location.href="{{url('admin/option_pharma')}}/"+ $(".volx").val();
