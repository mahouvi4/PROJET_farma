...
<div class="modal" tabindex="-1" id="ganix">
         ...
         <form id="op_cx">
          <div id="repax">
              <div class="group-control">
                   ...
                   <input type="radio"  name="comission" value="0.33" class="voxp">33%
                   <input type="hidden" value="{{session('info_boutique')[0]->id}}" class="id_phar">
              </div>

         </form>
      </div>
      <div class="modal-footer">
        <a href="{{url('alization/pagamento/'.session('info_boutique')[0]->id)}}"  style="width:100%"> <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" style="width:100%">Close</button></a>
      </div>
    </div>
  </div>
</div>
      ...
      <script>
                     ...
                     $(document).on("click",'.voxp',function(){
                        ...
                        // $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }});
                         $.ajax({
                             type:"get",
                             url:"{{url('admin/apply')}}/"+ $(".id_phar").val(),
                             data:{
                              comission: reco_p,
                             },
                             ...
                             success:function(response){
                                   if(response.status == 200){
                                    $("#repax").html(response.message).addClass("alert alert-info");

                                   }else{
                                    $("#repax").html("sorry").addClass("alert alert-danger");
