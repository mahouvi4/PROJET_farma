                ...
                <tbody>
                  ...
                  <td class="product-thumbnail">
                           <img src="{{asset('pharmacie/'.$xax->photo)}}" alt="" height="200px" width="100%" style="border-radius:15px">
                                     ...
                                     <td>@if(isset($tt)) <strong style="color:#317dda;font-family:cambria;font-size:3rem">${{$tt}}</strong> @else <strong style="color:#317dda;font-family:cambria;font-size:3rem">${{0}}</strong>  @endif  -
                                     
                                     @if(isset($devo->total_devolution_pharma))  <strong style="color:#ff0f5b;font-family:cambria;font-size:3rem">${{$devo->total_devolution_pharma}}</strong> @else <strong style="color:#ff0f5b;font-family:cambria;font-size:3rem">${{0}}</strong>  @endif -       
                                    
                                     @if(isset($comission)) <strong style="color:#313030;font-family:cambria;font-size:3rem">{{$comission->comission}}% </strong> @else  <strong style="color:#313030;font-family:cambria;font-size:3rem">{{0}}%</strong>   @endif =


                                     @if(isset($salaire)) <strong style="color:#313030;font-family:cambria;font-size:3rem">${{$salaire}} 👈 Current salary</strong> @else  <strong style="color:#313030;font-family:cambria;font-size:3rem">${{0}} 👈 Current salary</strong>     @endif 
                      ...
                      <thead>
                                  ...
                                  $created_at = Date("d-m-y",strtotime($h_acto));
                                        ...
                                        <tr>
                                          <div id="ilo"></div>
                                            ...
                                            @endif
                                            @if($item->statut == 4) 
                                            <td style="color:#ff0f5b;font-family:cammbria;background-color:#0b0b0b"> returned</td>
                                    @else
                                    @if($item->statut == 1)
                                   
                                       <td style="color:#fff;font-family:cambria;background-color:#05774a">Paid</td>
  
                                     @elseif($item->statut == 2)
                                     <td style="color:#080909;font-family:cambria;background-color:#056f77">Out for delivery. . .</td>
                                     
                                      @else @if($item->statut == 0)
                                     <td style="color:#DC143C;font-family:cambria;background-color:#ff0f5b">Awaiting payment. . .</td>
                                      @elseif($item->statut == 3 && $item->consolidation == 0)
                                     <td style="color:#67f5e7;font-family:cambria;background-color:#040e1d"> product Delivered. . .</td>
                                        @else
                                       
                                         <td style="color:#fff;font-family:cambria;background-color:#313030"> product Delivered + consolidated</td> 
                          ...
                          @endif          
                                            ...
                                            <a href="{{url('admin/detail_com_bout_adm/'.$item->id.'/'.session('info_boutique')[0]->id)}}" class="btn btn-primary height-auto btn-sm mt-2" style="width:100%;color:black">Detail Order</a>
                                              ...
                                              @if($item->statut <= 3 && $item->consolidation == 1)
                                               @elseif($item->statut <=  3 && $item->consolidation == 0)
                                               <button type="button" class="btn btn-primary height-auto btn-sm mt-2 xoxiii"   value="{{$item->id}}" style="width:100%;color:black" value="{{$item->id}}" >Update Status</button>
                                               @endif
