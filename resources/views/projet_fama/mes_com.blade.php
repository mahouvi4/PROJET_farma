        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 mb-0">
                    ...
                    <strong class="text-black">Orders List</strong>
                </div>
            </div>
        </div>
    </div>
 ...
 <div class="comodox"></div>
    ...
    @php
     ...
     $created_at = 0;
          ...
          @endphp
       <div class="content">
<marquee behavior="" direction="" style="font-family:Matura MT Script Capitals;color:rgb(121, 240, 224)"><h1>My orders😇 . . . . </h1></marquee>
     ...
     <div class="card">
        <div class="card-title" style="margin-left:0px;font-family:aharoni; color:rgba(10, 6, 22, 0.904)">
           ...
           <tr>
            ...
            <td style="text-align: center ;color:aqua">{{$item->code_commande}}</td>
                            
                                <td style="text-align: center;color:rgb(6, 114, 238)">{{$created_at}} {{$dato}}</td>
                    ...
                    <tr>
                    @if ($item->statut == 4)
                    <td style="text-align: center;color:crimson"><h2 style="color: rgb(6, 36, 36)">STATUS:</h2> value returned</td> 
                       ...
                       <td style="text-align: center;color:crimson"><h2 style="color: rgb(6, 36, 36)">STATUS:</h2> Waiting for payment . . .</td> 
                        @else
                           @if ($item->statut==1)
                           <td style="text-align: center;color:rgb(10, 136, 98)"><h2 style="color: rgb(6, 36, 26)">STATUS:</h2>Paid</td> 
                              
                               
                               @else

                               @if($item->statut == 2)
                               <td style="text-align: center;color:rgb(3, 53, 38)"><h2 style="color: rgb(6, 36, 36)">STATUS:</h2>Out for delivery . . .</td> 
                               @else
                       ...
                       @endif
                         @endif
                       <td style="font-size:1.5rem;color:rgba(115, 117, 117, 0.589)">Total: <h1 style="font-size:2.5rem;color:blue">${{number_format($item->total_commande, 2,".","")}}</h1></td>
                      
                      <td>
                       <h2 style="font-family:Franklin Gothic Medium ;margin-left:-45px">Detail:</h2> 
                      <button type="button" class="btn btn-primary id_cx" style="background-color:rgb(0, 38, 255);margin-left:-30px" value="{{$item->id}}">View</button>
                      <a href="{{url('admin/detail_cli_com/'.$item->id)}}" >.</a>
                       
                    <input type="hidden" class="idon" value="{{$item->pharmacie->id}}">
                   </td>
    ...
    <script>
        function destroy_session_user(){
         ...
         <script>
              ...
              /* $(function(){
                 $(".id_cx").on("click",{},function(){
                    var oio = $(this).val();
                 //alert(oio)
                $.ajax({
                  type : "get",
                  url : "{{url('admin/detail_cli_com')}}",
                  data:{
                     id_commande : $(this).val(),
                     id_pharmacie : $(".idon").val(),
                  },
                  success:function(response){
       ...
       <script>
        $(function(){
            $(".id_cx").on("click",function(){
                $.get("{{url('admin/detail_cli_com')}}/" + $(this).val(),{},function(data){
                   //console.log(data);
                   $("#elizof").html(data);
                   $("#g01").modal('show');
                })
