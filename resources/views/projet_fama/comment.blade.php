      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12 mb-0">
            ...
            <strong class="text-black"> Comment List</strong>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="site-section">
            ...
            <div class="site-blocks-table">
              ...
              @include('mood')
                <thead>
                  ...
                  <th class="product-name">Name Custumer</th>
                    <th class="product-total">Comment</th>
                    <th class="product-total">Pharmacy Contact</th>
                ...
                <tbody>
                   ...
                   <td>{{$item->imageuser->firstname.' '.$item->imageuser->name.'//-> '.$item->imageuser->email}}</td>
                   <td>{{$item->messages}}</td>
                   
                   @if($item->id_pharmacie)
                   <td style="color:green">{{$item->pharmacie->email}}</td>
                     @else
                     <td style="color:#ff0f5b;font-size:2rem">X</td>
                   @endif

                   @if($item->statut == 1)
                   <td style="color:green">Activated</td>
                   @else
                   <td style="color:red">Desactivated</td>
                
                   @endif
                      
                   <td>
                   @if($item->statut == 1)
                   <button type="button" class="btn btn-warning height-auto btn-sm mt-2 lok" style="width:100%" value="{{$item->id}}">Desactive Comment</button>
                      @else
                      <button type="button" class="btn btn-warning height-auto btn-sm mt-2 lok" style="width:100%" value="{{$item->id}}">Active Comment</button>

                   @endif
                  <button type="button" class="btn btn-danger height-auto btn-sm mt-2 " style="width:100%" onclick="delete__c({{$item->id}})">Delete</button>
      ...
      <div class="row">
          <div class="col-md-6">
            <div class="row mb-5">
              <div class="col-md-6 mb-3 mb-md-0">
                <a href="#"><button class="btn btn-primary btn-md btn-block">{{$comment->links()}}</button></a>
              </div>
              <div class="col-md-6">
              </div>
            </div>
                   ...
                   $(document).on('click','.lok',function(){
                    ...
                    //var ido = $(this).val();
                        //alert(momo);
                          $.ajax({
                                type:"get",
                                url:"{{url('admin/comment')}}/"+$(this).val(),
                      ...
                      <script>
                        function delete__c(id){
                             $.ajax({
                               type:"get",
                               url:"{{url('admin/delete_coment')}}/"+id,
                               success:function(response){
                                  if(response.status == 200){
