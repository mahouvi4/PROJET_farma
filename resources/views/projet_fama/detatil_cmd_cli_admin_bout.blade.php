            ...
            <div class="content">
                <div class="row">
                   <div class="card" style="background-color:rgba(3, 20, 26, 0.885)">
                    <div class="card-header"><p>Order Holder:</p><h2 style="color:rgba(55, 140, 238, 0.885);font-family:Matura MT Script Capitals">{{$xit->commande->name.' '.$xit->commande->firstname}}</h2></div>
                    <div class="card-body col-8">
                        ...
                        <p style="color:red;font-family:aharoni">returned</p>
                          ...
                          @endif
                        @endforeach
                        <p><h2 style="font-family:Matura MT Script Capitals;color:#ff0f5b">TOTAL:</div><strong style="font-size:2rem"> ${{number_format($xit->total_list, 2,".","")}}</strong></h2></p>
                        ...
                        @if($xut->verifier_statut_entrega == 1)
                                 @if($id_address_dif->receiv_user_com == NULL)
                                   <div class="card" style=" box-shadow: 0 25px 25px rgba(248, 248, 252, 0.885);text-align:center;font-family:Times New Roman;margin-top:-5px;border-radius : 25px;background-color:rgba(3, 20, 26, 0.885);color:rgb(20, 183, 233)" width="550px">
                                         <p>ORDER ADDRESS:<br> {{$xit->commande->country.', '.$xit->commande->state.', '.$xit->commande->city}}<br>{{$xit->commande->street.' '.$xit->commande->n_apto.', '.$xit->commande->zipcode}}<br> <a style="font-size:1.5rem;color:rgb(215, 228, 227)" href="{{url('admin/list_com_cli_bout_adm/'.$xit->pharmacie->id)}}">Return¬</a></p>
                                     ...
                                     <div class="card" style=" box-shadow: 0 25px 25px rgba(248, 248, 252, 0.885);text-align:center;font-family:Times New Roman;margin-top:-5px;border-radius : 25px;background-color:rgba(3, 20, 26, 0.885);color:rgb(20, 183, 233)" width="550px">
                                         ...
                                         <br>{{$id_address_dif->receiv_country.', '.$id_address_dif->receiv_state.', '.$id_address_dif->receiv_city}}<br>{{$id_address_dif->receiv_street.' '.$id_address_dif->receiv_npt.', '.$id_address_dif->receiv_zipcode}}<br> <a style="font-size:1.5rem;color:rgb(215, 228, 227)" href="{{url('admin/list_com_cli_bout_adm/'.$xit->pharmacie->id)}}">Return¬</a></p>
                            
                                   </div>
                                 @endif
                            @elseif($xut->verifier_statut_entrega == 0)
                            <div class="card" style=" box-shadow: 0 25px 25px rgba(248, 248, 252, 0.885);text-align:center;font-family:Times New Roman;margin-top:-5px;border-radius : 25px;background-color:rgba(3, 20, 26, 0.885);color:rgb(20, 183, 233)" width="550px">
                                         <p>ORDER ADDRESS:<br> The customer decided to go there [at the Farmacia to retrieve his order ♠]<br> <a style="font-size:1.5rem;color:rgb(215, 228, 227)" href="{{url('admin/list_com_cli_bout_adm/'.$xit->pharmacie->id)}}">Return¬</a></p>
